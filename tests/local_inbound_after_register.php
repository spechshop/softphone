...
Coroutine\run(function () use (&$exitCode): void {
    ...
    $invite = "INVITE sip:spechphone@127.0.0.1:4000 SIP/2.0\r\n"
    ...
    while (microtime(true) < $deadline) {
        ...
        if (in_array(180, $codes, true)) break;
    }

    $cancel = "CANCEL sip:spechphone@127.0.0.1:4000 SIP/2.0\r\n"
    ...
    $passed = in_array(100, $codes, true)
        && (in_array(180, $codes, true) || in_array(487, $codes, true));
    echo json_encode(['inbound_received_on' => 4000, 'response_codes' => array_values(array_unique($codes)), 'passed' => $passed]) . PHP_EOL;
    $exitCode = $passed ? 0 : 1;
});
exit($exitCode);

...
// Função para gerar ID único aleatório
...
// Tenta pegar o token da memória (localStorage), se não existir, gera um novo
(function() {
    let token = localStorage.getItem('fp');

    if (!token) {
        // Gera um novo token único
        token = generateUniqueDeviceId();
        localStorage.setItem('fp', token);
        console.log('🆔 Novo device ID gerado:', token);
    } else {
        console.log('🆔 Device ID existente:', token);
    }

    // Exibe o token na página
    const fpElement = document.getElementById('fp');
    if (fpElement) {
        fpElement.innerText = token;
    }
})()
checkTokenBrowser = setTimeout(async function () {
    const tokenBrowser = $('#fp').text()
    if (tokenBrowser) {
        $.ajax({
                type: 'POST',
                url: '/checkToken',
                dataType: 'json',
                data: {tokenBrowser: $('#fp').text()},
